...
`table_640x480`, 2000 measured frames per implementation, all three runs
back to back in one measurement window, milliseconds:
...
driver, that is the wrong trade, so the node sets the variable.

**Read the absolute figures as a band, not to three significant figures.** The
same binary on the same corpus measures between 56.6 and 60.7 ms p50 across
runs on this machine, about 12 percent, with no corresponding signal in the
load average: it is a shared vCPU whose throughput drifts over tens of minutes.
Run length is not the cause, which was checked (250, 500 and 2000 frame runs
all land in the same band). Every comparison in this repository therefore
measures both implementations inside one window, so the drift cancels in the
ratio even where it moves the absolutes.

**The C++ number is a flag choice, and the flags are worth more than the
language gap.** The table above uses the pinned `-O2 -DNDEBUG`. Rebuilt with
`-O3 -march=native`, the same C++ runs p50 33.3 ms rather than 53.0 ms in the
same window, 1.59x faster, which moves the ratio from about 1.6x to about 2.6x.
`-O3` alone is *slower* than `-O2` here (0.94x). `-march=native` enables FMA
contraction, so it changes the arithmetic and the binary; it changes the
answers by 1e-14, checked through the same equivalence gate, which is eight
orders of magnitude inside tolerance. See
[results/compiler_flags.json](results/compiler_flags.json), regenerated by
`harness/compiler_sweep.py`.
...
**The single ratio is one point on a curve, and the curve is the transferable
result.** The whole-pipeline ratio is modest only because one vectorised stage
dominates the frame. Vary the workload mix and it moves by a factor of five:

| configuration | plane stage, share of C++ frame | py:cpp at p50 |
| :--- | ---: | ---: |
| stride 1, 128 RANSAC candidates (shipped) | 95.5% | 1.59x |
| stride 2, 128 candidates | 95.2% | 1.71x |
| stride 4, 128 candidates | 94.2% | 2.17x |
| stride 1, 32 candidates | 85.8% | 4.20x |
| stride 1, 8 candidates | 68.9% | 7.89x |

The equivalence gate was re-run for every row and passes at 1.8e-13 or better,
so each point compares two programs computing the same thing. The mechanism is
in the numbers: cutting candidates from 128 to 8 divides the C++ plane stage by
10.5 and the Python one by only 1.95, because what remains on the Python side
is per-frame work over the whole cloud that does not shrink with the candidate
count. Ask "what does Python cost" and the honest answer is "between 1.6x and
7.9x, depending on how much of your frame is one large array operation".
See [results/workload_sweep.json](results/workload_sweep.json) and
[docs/METHOD.md](docs/METHOD.md) T1.

### Why the whole pipeline is 1.6x when single stages are 48x
