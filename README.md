...
`table_640x480`, 2000 measured frames per implementation over a 100-frame
recorded corpus, milliseconds:

| implementation | p50 | p95 | p99 |
| :--- | ---: | ---: | ---: |
| C++, Eigen 3.4, `-O2 -DNDEBUG` | 58.0 | 62.2 | 64.5 |
| Python, NumPy 2.4 and SciPy 1.17 | 86.9 | 96.7 | 107.2 |
| Python, `OPENBLAS_NUM_THREADS=1` | 85.8 | 91.8 | 98.7 |

Python costs **1.50x at p50 and 1.66x at p99**. Pinned to one BLAS thread it
costs 1.48x and 1.53x, which is the per-core comparison: the C++ build has no
OpenMP and used one core while NumPy's BLAS was free to use four. The p99
figures carry a 95% frame-blocked bootstrap interval, 63.8 to 65.2 ms for C++
and 103.2 to 112.3 ms for Python.

**Crossover.** The budget is 33.3 ms, one period at 30 Hz. C++ p99 crosses it
at about 162,020 pixels, roughly 465x349 at 4:3. Python p99 crosses at about
92,006 pixels, roughly 350x263. Both are linear interpolations of p99 against
pixel count between the two measured resolutions that bracket the budget: they
locate the crossing, they do not model it.

**Neither implementation holds 33.3 ms at 640x480.** 100% of frames are over
the budget in both. At 320x240 both hold it, with 0.0% of C++ frames and 0.1%
of Python frames over. Driven by a scheduler with absolute deadlines, the same
result: at 320x240 both run 30 Hz with no missed deadline, at 640x480 C++
misses every deadline at 30 Hz and Python misses 2.5% at 10 Hz. **At 640x480
the pipeline is the problem before the language is, and porting it to C++ does
not fix it.**

### Why the whole pipeline is 1.50x when single stages are 48x
...
headline: the sweep's own baseline row is the published configuration measured
again at 500 frames and it reads 1.59x rather than 1.50x, which is run-to-run
variation on a shared box and is the reason the sweep re-measures its own
baseline. The transferable part is not the six rows. It is the axis they are
